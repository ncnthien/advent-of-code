...
const replaceFirstNumberWithDigit = (line: string): string => {
  return line.replace(
    new RegExp(Object.keys(digitDictionary).join("|")),
    (matched) => {
...
const replaceLastNumberWithDigit = (line: string): string => {
  return line
    ...
    .join("")
    .replace(
      new RegExp(Object.keys(reversedDigitDictionary).join("|")),
      (matched) => {
...
const secondResult = lines.reduce((total, line) => {
  return (
    total +
    parseInt(
      `${getFirstNumberInLine(
        replaceFirstNumberWithDigit(line)
      )}${getFirstNumberInLine(replaceLastNumberWithDigit(line))}`
    )
  )
}, 0)
